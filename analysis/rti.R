...
# For getting visualizations run below after running summary statistics----
#removing extreme outlier from synchronous condition
rti.long.data <- rti.long.data[-326,]

# descriptive statistics
describeBy(rti.long.data, rti.long.data$condition)

## data frame of rti summary by day
summary.rti <- rti.long.data %>%   
  group_by(condition, day) %>% 
  summarize(
    mean.elongation = mean(elongation),
    sd.elongation = sd(elongation),
    count = n()) %>% 
  mutate(experiment = case_when(
    endsWith(condition, "control") ~ "control",
    endsWith(condition, "Synchronous") ~ "stroking",
...
#changing facet grid o that the names of experiments are capitalized
...
rti.total <- ggplot(rti.long.data, aes(x = condition, y = elongation)) +
  ...
  theme(text = element_text(family="arial"),
        ...
        panel.border = element_rect(color = "#9C1F2E", fill = NA, size = .05),
        strip.text = element_text(face = "bold", size = 25, colour = "#9C1F2E"),
        strip.text.x = element_text(size = 15),
        strip.background = element_rect(fill = "#7F7F7F"),
  ...
  geom_signif(comparisons=list(c("Asynchronous", "Synchronous")), annotations = c("*"),
              y_position = 83, tip_length = 0, vjust=0.4)


rti.by.day <- ggplot(summary.rti, aes(x = condition, y = mean.elongation)) + 
  stat_boxplot(geom ='errorbar', width = 0.5) +
  geom_boxplot(width = 0.5, fill = "lightgrey") +
  geom_jitter(color="darkgrey", size=0.4, alpha=0.9) +
  geom_violin(alpha = .05) +
  facet_grid(~experiment, scales = "free", space = "free") +
  theme_bw(base_size = 10, base_family = "BOOKOS") +
  theme(strip.text.y = element_text(),
        strip.background = element_rect(fill = "#9C1F2E")) + 
  stat_n_text() + 
  labs(x = "Condition", y = "Body Elongation (%)") +
  scale_y_continuous(expand = expansion(mult = c(0.05, 0.15)))


#for separation by sex, use geom_point(position=position_jitterdodge())

# For getting repeated measures ANOVA by day run below
## stroking by day
